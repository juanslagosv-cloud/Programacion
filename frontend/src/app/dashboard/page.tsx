...
import { accesosRapidosParaRol } from "@/lib/roles";
import { obtenerSesion, type SesionDemo } from "@/lib/session";

// Dashboard personalizado por rol (sección 5): "Hola, [Nombre]" + accesos rápidos
// según el rol activo. La tarjeta de estado del sistema demuestra la conexión
// real Frontend -> API (nunca Frontend -> Access/Drive directamente, sección 3/61).
export default function DashboardPage() {
  ...
  return (
    <div className="max-w-3xl">
      <h1 className="text-2xl font-semibold text-slate-900">Hola, {sesion.nombre}</h1>
      <p className="mt-1 text-sm text-slate-500">Rol activo: {sesion.rol}</p>

      <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {accesosRapidosParaRol(sesion.rol).map((acceso) => (
          <div
            key={acceso}
